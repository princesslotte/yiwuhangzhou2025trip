...
import { TravelEvent, Translation } from '../types';
...
const EventModal: React.FC<EventModalProps> = ({ isOpen, onClose, onSave, initialEvent, t }) => {
  ...
  const [category, setCategory] = useState<TravelEvent['category']>('sightseeing');
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      notes,
      category,
      // Preserve existing mapQuery if we are editing and it exists, 
      // otherwise new events will rely on the default behavior in App.tsx (using location name)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-[#594a40]/20 backdrop-blur-sm transition-all duration-300">
      <div className="bg-[#f8e8c1] w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-6 shadow-soft text-[#594a40] border border-white/50 relative overflow-hidden">
        ...
        <div className="relative z-10">
            ...
            <h2 className="text-2xl font-black">{initialEvent ? t.editEvent : t.addEvent}</h2>
