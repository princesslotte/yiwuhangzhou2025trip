...
import { X, ExternalLink, Calendar, MapPin, CheckSquare, Square, RefreshCcw } from 'lucide-react';
import { Translation, Language } from '../types';
import { HOTEL_DATA, PREP_ITEMS, USEFUL_INFO } from '../constants';

interface MenuModalProps {
  type: 'hotel' | 'prep' | 'info' | 'currency' | null;
  onClose: () => void;
  lang: Language;
  t: Translation;
}
...
const MenuModal: React.FC<MenuModalProps> = ({ type, onClose, lang, t }) => {
  ...
  const [amountCNY, setAmountCNY] = useState('100');
  const [amountKRW, setAmountKRW] = useState('');
  const [rate, setRate] = useState(193); // Default approx rate
          ...
          <div key={idx} className="bg-white p-5 rounded-3xl shadow-journal border border-white relative overflow-hidden group hover:scale-[1.01] transition-transform">
            ...
            <div className="flex justify-between items-start mb-3">
               <div className="flex items-center gap-2">
                  <span className="bg-[#594a40] text-[#f8e8c1] text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider">{item.city}</span>
        ...
        {PREP_ITEMS[lang].map((item, idx) => {
          ...
          return (
            <div 
              ...
              className={`flex flex-col p-5 bg-white rounded-3xl border-2 cursor-pointer transition-all duration-200 ${isChecked ? 'border-[#594a40]/10 bg-slate-50 opacity-80' : 'border-[#594a40]/20 hover:border-[#594a40]/40 shadow-sm'}`}
            >
        ...
        <div className="space-y-3">
          {USEFUL_INFO[lang].map((item, idx) => (
            ...
            >
               ...
               <span className="text-sm font-medium bg-[#f8e8c1]/50 px-2 py-1 rounded text-[#594a40] group-hover:bg-[#f8e8c1] transition-colors">
                 {item.value}
               </span>
            </a>
          ))}
    ...
    content = (
      <div className="space-y-6">
        ...
        <div className="text-center">
           ...
           >
             {t.checkXe} <ExternalLink size={12} />
           </a>
        </div>
      </div>
    );
